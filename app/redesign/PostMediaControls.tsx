...
export default function PostMediaControls({
  ...
  return (
    <div className="mq-post-media-section">
      ...
      {phase !== "idle" ? (
        <div className="mq-post-media-workflow" role="status" aria-live="polite">
          <div>
            {phase === "uploading" ? <Upload size={15} /> : <LoaderCircle className="mq-spin" size={15} />}
            <strong>{phase === "preparing" ? "Preparing selected image…" : phase === "uploading" ? `Uploading media · ${uploadProgress ?? 0}%` : "Upload complete. Waiting for media to be ready…"}</strong>
          </div>
          {phase === "uploading" ? <div className="mq-post-media-progress" role="progressbar" aria-label="Media upload progress" aria-valuemin={0} aria-valuemax={100} aria-valuenow={uploadProgress ?? 0}><span style={{ width: `${uploadProgress ?? 0}%` }} /></div> : null}
        </div>
      ) : null}
      {!readOnly ? <div className="mq-post-media-actions">
